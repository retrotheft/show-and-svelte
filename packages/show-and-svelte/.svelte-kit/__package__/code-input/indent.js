export function createIndentPlugin(useSpaces = false, indentSize = 4, bracketPairs = { '(': ')', '[': ']', '{': '}' }) {
    const indentation = useSpaces ? ' '.repeat(indentSize) : '\t';
    const indentationLength = useSpaces ? indentSize : 1;
    return {
        keydown(event, textarea) {
            if (event.key === 'Tab') {
...
function checkTab(event, textarea, indentation, indentationLength) {
...
function checkEnter(event, textarea, indentation, indentationLength, bracketPairs) {
    event.preventDefault();
...
function checkBackspace(event, textarea, indentation, indentationLength) {
    const { selectionStart, selectionEnd, value } = textarea;
...
function checkCloseBracket(event, textarea, indentation, indentationLength, bracketPairs) {
    if (!event.data)
        return;
    const { selectionStart, value } = textarea;
    const lines = value.split('\n');
